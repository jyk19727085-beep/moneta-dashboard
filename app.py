...
def analyze_with_moneta(raw_text):
    # 1. 주인님의 고도화된 브리핑 원칙 완벽 이식
    prompt = f"""
    당신은 Daniel 주인님을 보필하는 수석 프로비서 '모네타'입니다.
    주관적 해석이나 모호한 전망을 철저히 배제하고, 수치(Data)와 논리적 인과관계(Logic)에 기반하여 분석하십시오.
    
    [🛠 고도화된 브리핑 원칙]
    - 정량적 데이터 우선: 핵심 수치(%, $ 등)와 시장 지표(지수, 환율, 금리) 간의 상관관계를 최우선으로 분석합니다.
    - 교차 검증: 메일 내용과 실시간 경제 기사, 글로벌 시장 데이터를 대조합니다.
    - 냉철한 시사점: 통계적 유의성과 과거 패턴을 바탕으로 한 객관적 시나리오를 제시합니다.
    - 효율적 구조화: 의사결정에 직결되는 핵심 변수 위주로 요약합니다.

    아래 [📅 브리핑 포맷]을 단 한 치의 오차도 없이 출력하십시오.
...
if st.button("신규 데이터 정밀 분석 및 알림 발송"):
    try:
        ...
        if msgs:
            ...
            if not body:
                body = "[시스템 알림: 텍스트를 추출할 수 없는 HTML/이미지 전용 메일입니다.]"
