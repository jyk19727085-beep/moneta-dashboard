...
def analyze_with_moneta(raw_text):
    # 주인님의 절대 헌법 [고도화된 브리핑 원칙 및 포맷] 완벽 이식
    prompt = f"""
    ...
    - 교차 검증 (Cross-Check): 메일 보고서 내용과 실시간 경제 기사, 글로벌 시장 데이터를 대조하여 신뢰도를 평가합니다.
    ...
    # 🚨 구글 최신 AI 엔진(gemini-1.5-pro)으로 버전업 완료
    ...
    try:
        ...
        # 에러 발생 시 숨기지 않고 화면에 강제 표출하여 즉시 대응
...
if st.button("신규 데이터 정밀 분석 및 알림 발송"):
    try:
        ...
        if msgs:
            ...
            if not body:
                body = "[시스템 알림: 텍스트를 추출할 수 없는 구조의 메일입니다.]"
